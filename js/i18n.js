...
const I18nManager = {
  ...
  // Determinar la ruta correcta para los locales
  getLocalesPath() {
    const currentPath = window.location.pathname;

    // Si estamos en la carpeta pages (como agencias.html)
    if (currentPath.includes("/pages/")) {
      return "../locales";
    }

    // Si estamos en la raíz (como index.html)
    return "locales";
  },
  // Inicialización del módulo
  async init() {
    try {
      this.config.localesPath = this.getLocalesPath();
  ...
  translatePage() {
    ...
    // if (currentPath.includes('agencias.html')) {
    //     titleKey = 'header.titleAgencies';
    // }
